...
case class Inexact(private val _value: Double) extends Numeric with Ordered[Value] {
  ...
  override def /(other: Numeric): Numeric =
    other match {
      ...
      case x: Exact => if (x.value == 0) throw new IllegalValueException("Divide by 0!") else Inexact(this.value / x.value)
      case _ => throw new TypeException("Numeric operand required")
    }
